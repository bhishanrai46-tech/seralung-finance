...
def parse_bank_csv(f):
    try:
        ...
        ncol=next((c for c in df.columns if any(k in c for k in ["description","details","narration","memo"])),None)
        ...
        if acol:
            ...
            res["Amount"]=c2-d2
        else: return None,"Cannot find Amount columns."
...
[data-testid="stSidebar"] .stButton>button{{
  background:transparent!important;border:1.5px solid {BO}!important;
  color:{TX}!important;font-weight:500!important;border-radius:8px!important;
  font-size:.78rem!important;transition:all .15s;
}}
...
with t5:
    ...
    with rp2:
        ...
        bank_file=st.file_uploader("Drop your bank CSV here",type=["csv"],key="bank_up")
        if bank_file:
